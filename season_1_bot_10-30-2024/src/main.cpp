...
void blue_detected() {
  if (Optical1.color() == blue){
    if( (intake_tracking.angle() < 200) && (intake_tracking.angle() > 0) ) {
    intake.spin(forward);
    }
    else{
      intake.stop(hold);
    ...
    if( (intake_tracking.angle() > 55) && (intake_tracking.angle() < 350) ) {
  intake.spin(forward);
    }
    else{
      intake.stop(hold);  
...
void red_detected() {
  if (Optical1.color() == red){
    if ((intake_tracking.position(degrees)  ==  127)  or ( intake_tracking.position(degrees) == 161)  or ( intake_tracking.position(degrees) == 119)   or ( intake_tracking.position(degrees) == 66)) {
     intake.stop(hold);
    }
  }
}
